...
#print(os.getcwd()) #/home/kizunasunhy/khaiii/train
users_dic_path = './rsc/preanal.manual'
#users_dic_path = '/home/kizunasunhy/khaiii/rsc/src/preanal.manual'
users_dic = _load_users_dict(users_dic_path)
...
class PosWord:
    ...
    def set_pos_result(self, tags: List[str], restore_dic: dict = None):
        ...
        self.raw = raw
        #print(self.res_chrs, self.res_tags)
    ...
    def _make_pos_morphs(self, restore_dic: dict = None):
        ...
        for beg, (lex, iob_tag) in enumerate(zip(self.res_chrs, self.res_tags)):
            ...
            if iob in self.dict:
                if ' + ' in users_dic[self.dict[iob]]:
                    pair_list = users_dic[self.dict[iob]].split(' + ')
                    for pair in pair_list:
                        pair_a, pair_b = pair.split('/')
                        pos_morphs.append(PosMorph(pair_a, pair_b))
                else:
                    raw, mor = users_dic[self.dict[iob]].split('/')
                    pos_morphs.append(PosMorph(self.dict[iob], mor))
            elif iob == 'B' or not pos_morphs or pos_morphs[-1].pos_tag != pos_tag:
    ...
    def _restore(self, restore_dic: dict):
        ...
        for key in users_dic:
            if key in self.raw: 
                ...
                index_start = self.raw.find(key)                    
                self.raw = self.raw.replace(key, str(number))
                self.dict[str(number)] = key
                self.tags[index_start:(index_start+len(key))] = str(number)
                number += 1
...
class PosSentence(Sentence):
    ...
    def set_pos_result(self, tags: List[str], restore_dic: dict = None):
        ...
        for pos_word in self.pos_tagged_words:
            #print(tags[total_char_num:total_char_num + len(pos_word.raw)])
